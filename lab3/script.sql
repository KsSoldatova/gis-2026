...
osm_data_bbox AS (
    SELECT ST_Xmin(geom) - 0.01 AS xmin,
           ST_Ymin(geom) - 0.01 AS ymin,
           ST_Xmax(geom) + 0.01 AS xmax,
           ST_Ymax(geom) + 0.01 AS ymax
...
SET source_type = CASE
    WHEN EXISTS (
        ...
        WHERE try(ST_Intersects(o.geom, ST_SetCRS(ob.geometry, 'EPSG:4326'))) = true
    ) THEN 'my'
    WHEN list_contains(list_transform(ob.sources, s -> s.dataset), 'OpenStreetMap') THEN 'osm'
    ELSE 'ml'
END;
